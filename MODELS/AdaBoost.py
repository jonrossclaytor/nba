...
for p in player_list:
    ...
    for target in [assists_str,rebounds_str,points_str,steals_str,blocks_str,turnovers_str,made_threes_str]:
        ...
        rf_test_float = rf_test.astype(np.float)

        MSE = math.sqrt(sum((rf_test_float - target_test_float)**2)) / rf_test_float.shape[0]

        result = [p,today,'randomforest',stat,MSE,0,'Not Applicable']
        result = tuple(result)
        result = tuple(result)
